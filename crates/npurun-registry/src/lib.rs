...
//!   inspect the embedded `genie_config.json`, and emit a `npurun.json`
//!   manifest so the model is ready for `npurun run`.
//! - **List local**: walks the cache for `npurun.json` files.
...
pub enum RegistryError {
    ...
    #[error(transparent)]
    Manifest(#[from] npurun_core::ManifestError),
}

/// Resolve the default model cache directory.
///
/// On Windows the default is `%LOCALAPPDATA%\npurun\models`. If
/// `NPURUN_MODELS_DIR` is set, it overrides. If neither is set, the
/// working directory's `models/` is used as a fallback.
pub fn default_cache_dir() -> PathBuf {
    if let Some(d) = std::env::var_os("NPURUN_MODELS_DIR") {
        return PathBuf::from(d);
    }
    if let Some(local) = std::env::var_os("LOCALAPPDATA") {
        let p = PathBuf::from(local).join("npurun").join("models");
...
pub async fn list_local() -> Result<Vec<String>, RegistryError> {
    ...
    for entry in read {
        ...
        let path = entry.path();
        if path.is_dir() && path.join("npurun.json").is_file() {
...
///
/// Refuses to delete anything that doesn't look like a npurun model
/// directory (must contain a `npurun.json`) — defensive guard against
...
pub fn remove_local(model_name: &str) -> Result<PathBuf, RegistryError> {
    ...
    let manifest = dir.join("npurun.json");
    if !manifest.is_file() {
...
mod tests {
    ...
    fn default_cache_dir_uses_models_dir_env_when_set() {
        if std::env::var_os("NPURUN_MODELS_DIR").is_some() {
