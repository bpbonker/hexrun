//! `pull_model` — download a known bundle, extract it, write a npurun.json.
//!
...
use npurun_core::{ChatTemplate, Manifest, ManifestFiles, Quant};
...
pub enum ProgressEvent {
    ...
    Extracting,
    /// npurun.json has been written; pull is complete.
    Done {
...
/// `<cache>/<name>/bundle/`. A `npurun.json` is written at
/// `<cache>/<name>/npurun.json`.
///
...
/// Auto-generate the npurun.json from the extracted bundle.
fn write_manifest(
    known: &KnownModel,
    model_dir: &Path,
    bundle_root: &Path,
