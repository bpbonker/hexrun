...
impl ServerState {
    ...
    /// Builder helper used by `npurun serve --model <name>`.
...
/// Build the axum router with all npurun endpoints.
///
...
async fn require_auth(
    ...
    if !ok {
        return (
            ...
            [("www-authenticate", "Bearer realm=\"npurun\"")],
...
///
/// npurun does not version models the way Ollama does — each pulled
...
pub async fn serve(addr: SocketAddr, state: ServerState) -> Result<()> {
    ...
    tracing::info!(%addr, "npurun server listening");
    axum::serve(listener, app)
        .with_graceful_shutdown(shutdown_signal())
        .await?;
    tracing::info!("npurun server stopped cleanly");
    Ok(())
}
