"""Wrap qai-hub-models export for hexrun.
...
- A curated map from a hexrun-friendly slug (``phi-3.5-mini``) to the
...
- Post-processing: locate the produced bundle, emit a hexrun.json
  via ``hex_convert.manifest``.
...
def export(opts: ExportOptions) -> Path:
    """Run a qai-hub-models export and emit a hexrun.json.

    Returns the path to the written hexrun.json on success.
    """
...
def _check_prereqs() -> None:
    if platform.machine().lower() in ("arm64", "aarch64"):
        ...
        raise ExportError(
            ...
            "bundle can then be used by the ARM64 hexrun runtime."
        )
...
def _run_qai_hub_export(recipe: _Recipe, quant: str, output: Path) -> None:
    ...
    print(f"[hex-convert] launching: {' '.join(cmd)}", file=sys.stderr)
    try:
