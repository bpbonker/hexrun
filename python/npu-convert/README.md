...
Sidecar tool that turns HuggingFace LLMs into npurun-compatible
bundles. Three subcommands; you'll mostly use `manifest` and `inspect`.

```
npu-convert manifest --model-dir <dir> --bundle-dir <bundle> --name <slug>
npu-convert inspect <bundle-or-manifest>
npu-convert export <slug> --output <dir>      # heavy: requires AI Hub token
```
...
- **You already have a Genie bundle** (downloaded from Qualcomm's
  ...
  the runtime to load it. Run `npu-convert manifest` to drop a
  `npurun.json` next to it. Done.
- **You want to verify a npurun bundle's integrity** or see what's in
  it without parsing JSON by hand. Run `npu-convert inspect`.
- **You want to add a new model that's not in the built-in registry**
  and you have an AI Hub API token + an x64 Python venv + a few
  spare hours. Run `npu-convert export`.
...
6. Writes `<model-dir>/npurun.json`.
...
```powershell
npu-convert inspect C:\AAA\Personal\AI\models\phi-3.5-mini
npu-convert inspect C:\path\to\unmanaged-genie-bundle\
npu-convert inspect path\to\npurun.json --no-verify
```

If the path contains a `npurun.json`, it pretty-prints the manifest,
...
npu-convert export phi-3.5-mini --output C:\AAA\Personal\AI\models\phi-3.5-mini-fresh
```
...
- Then `npu-convert export` chains into `manifest` to drop a
  `npurun.json` on top.
...
ones is a matter of adding a recipe to `npu_convert/export.py`; see
...
- Downstream of `export`, the bundle is identical in shape to what
  `npurun pull` lands. The two flows produce interchangeable
  on-disk layouts.
