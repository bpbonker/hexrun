"""npu-convert CLI entry point.

Three subcommands:

- ``npu-convert manifest`` — emit a npurun.json over an existing Genie
  ...
  make it ``npurun pull``-equivalent on disk.
- ``npu-convert inspect`` — pretty-print a npurun bundle or a raw
  Genie bundle, optionally verifying sha256s.
- ``npu-convert export`` — orchestrate a full HF -> ONNX -> AI-Hub
...
def main() -> None:
    """Convert HuggingFace LLMs into npurun-compatible bundles."""
...
@click.option(
    ...
    required=True,
    help="Directory the npurun.json will be written into.",
)
@click.option(
    "--bundle-dir",
...
def manifest_cmd(
    ...
    """Emit npurun.json over an existing Genie bundle."""
    ...
    try:
        ...
        click.echo(f"npu-convert manifest: {e}", err=True)
        sys.exit(2)
...
def inspect_cmd(target: Path, no_verify: bool) -> None:
    """Pretty-print a npurun bundle, npurun.json, or raw Genie bundle."""
...
@click.option(
    ...
    type=click.Path(file_okay=False, dir_okay=True, path_type=Path),
    required=True,
    help="Output directory; the npurun.json + bundle land here.",
)
@click.option(
    "--quant",
    default=None,
...
def export_cmd(
    ...
    """Run qai-hub-models export for SLUG and emit a npurun bundle.
    ...
    try:
        ...
        click.echo(f"npu-convert export: {e}", err=True)
        sys.exit(2)
