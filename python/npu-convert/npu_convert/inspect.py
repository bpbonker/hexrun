"""Pretty-print and verify npurun bundles and Genie bundles.

``npu-convert inspect`` is the diagnostic counterpart to ``manifest``:
read what a bundle / manifest claims to contain, and check that what's
on disk matches.

Three modes, picked from the path you point at:

- A directory containing ``npurun.json``: full manifest read,
  sha256 verify, list of files with sizes.
- A directory containing ``genie_config.json`` but no
  ``npurun.json``: report what we'd write a manifest with.
- A file path ending in ``npurun.json``: same as the first mode.
"""
...
def inspect(target: Path, *, verify: bool = True) -> int:
    ...
    if target.is_dir():
        ...
        if cfg is None:
            console.print(
                f"[red]no npurun.json or genie_config.json found under {target}[/red]"
            )
            return 1
...
def _inspect_genie_only(cfg_path: Path, *, console: Console) -> int:
    ...
    console.print(
        f"[yellow]no npurun.json — showing inferred manifest for the Genie bundle at "
        f"{bundle_dir}[/yellow]"
    )
    console.print()
    try:
    ...
    console.print()
    console.print(
        "[dim]write a npurun.json with: npu-convert manifest --bundle-dir "
