...
The npurun runtime needs a ``npurun.json`` manifest on top of that bundle
so it can validate the SDK version, look up the chat template, and verify
file integrity. ``npu-convert manifest`` writes that file.

This module is the canonical place for "what does a npurun manifest look
like for a Genie bundle" — the Rust ``npurun-registry`` crate does the
...
# Per-architecture chat templates. Mirrors npurun-registry/src/known.rs.
...
class ManifestOptions:
    """All inputs needed to write a npurun.json over a Genie bundle."""

    # The model directory we're going to write `npurun.json` into. The
...
def write_manifest(opts: ManifestOptions) -> Path:
    """Write a npurun.json into ``opts.model_dir``. Returns its path."""
    ...
    manifest_path = opts.model_dir / "npurun.json"
