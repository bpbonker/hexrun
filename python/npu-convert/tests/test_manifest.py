"""Tests for npu_convert.manifest.
...
from npu_convert.manifest import ManifestOptions, write_manifest
...
def test_writes_manifest_with_expected_fields(tmp_path: Path) -> None:
    ...
    assert path == tmp_path / "npurun.json"
...
def test_sha256_matches_actual_file_content(tmp_path: Path) -> None:
    ...
    m = json.loads((tmp_path / "npurun.json").read_text(encoding="utf-8"))
    sha = m["sha256"]
